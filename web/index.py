...
def ars():
    ...
    basetime = request.values.get("basetime", type=str, default=arrow.now())
    tn = TimeNormalizer(pattern, holiday, isPreferFuture=True)
    tn.parse(unicode(query), timeBase=unicode(basetime))
    res = ''
    cnt = 1
    for token in tn.timeToken:
        res += '时间点' + str(cnt) + '> ' + token.time.format("YYYY-MM-DD HH:mm:ss") + '\n'
        cnt += 1
    res = {'res': res}
    print res
...
@app.route('/time/analysis', methods=['GET', 'POST'])
def analysis():
    query = request.values.get("pattern", type=str, default=None)
    basetime = request.values.get("basetime", type=str, default=arrow.now())
...
def init():
    ...
    time_normalizer = TimeNormalizer(pattern, holi_solar, holi_lunar, isPreferFuture=True)
