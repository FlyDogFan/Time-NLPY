...
#
# t = arrow.get('2013-02-28-16-30-29', "YYYY-M-D-H-m-s")
# print t
...
### 測試輸出文件
def analysis(p, basetime, holi_solar, holi_lunar):
    tn = TimeNormalizer(pattern, holi_solar, holi_lunar, isPreferFuture=True)
    tn.parse(unicode(p), timeBase=unicode(basetime))
    dic = {}
    res = tn.timeToken
    if tn.isTimeSpan:
        dic['type'] = 'timedelta'
        dic['timedelta'] = tn.timeSpan
    else:
        if len(res) == 0:
...
with open('C:/Users/zhm/Desktop/test.txt') as testfile:
    ...
    for each in testfile:
        res = analysis(each, arrow.now(), holi_solar, holi_lunar)
        res_b = boson_analy(each, datetime.datetime.now())
        data.append(each+'Boson:'+res_b+'\n'+'Time-NLP:'+res+'\n'+'\n')
with open('C:/Users/zhm/Desktop/时间转换结果对比.txt', 'wb') as resfile:
...
def process():
    with open('../resource/holiday.json', 'rb') as f:
        res = {}
        reg = []
        str = ''
        for each in f:
            dic = json.loads(each)
            year = dic['date'].split('-')[0]
            dic['name'] = year + '-' + dic['name']
            dic['name'] = dic['name'].replace('假期', '').replace('中国', '').replace('黄金周', '').replace('（鬼节）', '').replace('国际', '')
            res[dic['name']] = dic['date']
            reg.append(dic['name'].split('-')[1])
            # print (dic['name'] + '--' + dic['date'])
    with open('../resource/holiday.txt', 'wb') as f:
        json.dump(res, f)
    reg = set(reg)
    for i in reg:
        str += '(' + i + ')' + '|'
...
def holi():
    solar = {}
    lunar = {}
    with open('../resource/holi_solar.txt', 'rb') as f:
        for each in f:
            arr = each.split()
            solar[arr[0]] = arr[1]
    with open('../resource/holi_lunar.txt', 'rb') as f:
        for each in f:
            arr = each.split()
            lunar[arr[0]] = arr[1]
    with open('../resource/holi_solar.json', 'wb') as f:
        json.dump(solar, f)
    with open('../resource/holi_lunar.json', 'wb') as f:
        json.dump(lunar, f)
